# https://gist.github.com/secdevlowe/c3ae773b9c2a45d91d7df59ba07b5d35
# Credit and thank you to https://collinmbarrett.com/protonvpn-dd-wrt-api-script/
# Store 2x copies of this file.
#   1. /opt/vpn-refresh.sh
#   2. /jffs/etc/config/vpn-refresh.wanup

# cat >> vpn-refresh.wanup <<EOL
# line 1...
# line 2...
# line 3...
# EOL

# chmod 700 /opt/vpn-refresh.sh
# chmod 700 /jffs/etc/config/vpn-refresh.wanup
...
SERVER=$(echo "$LOGICALS" | jq '.LogicalServers | map(select(.Status == 1 and .Tier == 2 and .City != null and (.City | (contains("Denver") or contains("Dallas") or contains("Chicago"))))) | [sort_by(.Score, .Load)[]][0] | .Servers[0]')
